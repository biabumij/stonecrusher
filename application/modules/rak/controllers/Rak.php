...
class Rak extends Secure_Controller {
	...
	public function form_rencana_kerja()
	{
		...
		if ($check == true) {
			...
			$data['boulder'] = $this->pmm_model->getMatByPenawaranRencanaKerjaBoulder();
			$data['solar'] = $this->pmm_model->getMatByPenawaranRencanaKerjaSolar();
			$data['tangki'] = $this->pmm_model->getMatByPenawaranRencanaKerjaTangki();
			$data['sc'] = $this->pmm_model->getMatByPenawaranRencanaKerjaSC();
			$data['genset'] = $this->pmm_model->getMatByPenawaranRencanaKerjaGNS();
			$data['wl'] = $this->pmm_model->getMatByPenawaranRencanaKerjaWL();
			$data['timbangan'] = $this->pmm_model->getMatByPenawaranRencanaKerjaTMB();
	...
	public function submit_rencana_kerja()
	{
		...
		$penawaran_id_tangki =  $this->input->post('penawaran_id_tangki');
		$penawaran_id_sc =  $this->input->post('penawaran_id_sc');
		$penawaran_id_genset =  $this->input->post('penawaran_id_genset');
		$penawaran_id_wl =  $this->input->post('penawaran_id_wl');
		$penawaran_id_timbangan =  $this->input->post('penawaran_id_timbangan');

		$harga_boulder =  str_replace('.', '', $this->input->post('price_boulder'));
		$harga_solar =  str_replace('.', '', $this->input->post('price_solar'));
		$harga_tangki =  str_replace('.', '', $this->input->post('price_tangki'));
		$harga_sc =  str_replace('.', '', $this->input->post('price_sc'));
		$harga_genset =  str_replace('.', '', $this->input->post('price_genset'));
		$harga_wl =  str_replace('.', '', $this->input->post('price_wl'));
		$harga_timbangan =  str_replace('.', '', $this->input->post('price_timbangan'));

		$satuan_boulder =  $this->input->post('measure_boulder');
		$satuan_solar =  $this->input->post('measure_solar');
		$satuan_tangki =  $this->input->post('measure_tangki');
		$satuan_sc =  $this->input->post('measure_sc');
		$satuan_genset =  $this->input->post('measure_genset');
		$satuan_wl =  $this->input->post('measure_wl');
		$satuan_timbangan =  $this->input->post('measure_timbangan');

		$tax_id_boulder =  $this->input->post('tax_id_boulder');
		$tax_id_solar =  $this->input->post('tax_id_solar');
		$tax_id_tangki =  $this->input->post('tax_id_tangki');
		$tax_id_sc =  $this->input->post('tax_id_sc');
		$tax_id_genset =  $this->input->post('tax_id_genset');
		$tax_id_wl =  $this->input->post('tax_id_wl');
		$tax_id_timbangan =  $this->input->post('tax_id_timbangan');

		$pajak_id_boulder =  $this->input->post('pajak_id_boulder');
		$pajak_id_solar =  $this->input->post('pajak_id_solar');
		$pajak_id_tangki =  $this->input->post('pajak_id_tangki');
		$pajak_id_sc =  $this->input->post('pajak_id_sc');
		$pajak_id_genset =  $this->input->post('pajak_id_genset');
		$pajak_id_wl =  $this->input->post('pajak_id_wl');
		$pajak_id_timbangan =  $this->input->post('pajak_id_timbangan');

		$supplier_id_boulder =  $this->input->post('supplier_id_boulder');
		$supplier_id_solar =  $this->input->post('supplier_id_solar');
		$supplier_id_tangki =  $this->input->post('supplier_id_tangki');
		$supplier_id_sc =  $this->input->post('supplier_id_sc');
		$supplier_id_genset =  $this->input->post('supplier_id_genset');
		$supplier_id_wl =  $this->input->post('supplier_id_wl');
		$supplier_id_timbangan =  $this->input->post('supplier_id_timbangan');
		...
		$arr_insert = array(
			...
			'penawaran_id_tangki' => $penawaran_id_tangki,
			'penawaran_id_sc' => $penawaran_id_sc,
			'penawaran_id_genset' => $penawaran_id_genset,
			'penawaran_id_wl' => $penawaran_id_wl,
			'penawaran_id_timbangan' => $penawaran_id_timbangan,

			'harga_boulder' => $harga_boulder,
			'harga_solar' => $harga_solar,
			'harga_tangki' => $harga_tangki,
			'harga_sc' => $harga_sc,
			'harga_genset' => $harga_genset,
			'harga_wl' => $harga_wl,
			'harga_timbangan' => $harga_timbangan,

			'satuan_boulder' => $satuan_boulder,
			'satuan_solar' => $satuan_solar,
			'satuan_tangki' => $satuan_tangki,
			'satuan_sc' => $satuan_sc,
			'satuan_genset' => $satuan_genset,
			'satuan_wl' => $satuan_wl,
			'satuan_timbangan' => $satuan_timbangan,

			'tax_id_boulder' => $tax_id_boulder,
			'tax_id_solar' => $tax_id_solar,
			'tax_id_tangki' => $tax_id_tangki,
			'tax_id_sc' => $tax_id_sc,
			'tax_id_genset' => $tax_id_genset,
			'tax_id_wl' => $tax_id_wl,
			'tax_id_timbangan' => $tax_id_timbangan,

			'pajak_id_boulder' => $pajak_id_boulder,
			'pajak_id_solar' => $pajak_id_solar,
			'pajak_id_tangki' => $pajak_id_tangki,
			'pajak_id_sc' => $pajak_id_sc,
			'pajak_id_genset' => $pajak_id_genset,
			'pajak_id_wl' => $pajak_id_wl,
			'pajak_id_timbangan' => $pajak_id_timbangan,

			'supplier_id_boulder' => $supplier_id_boulder,
			'supplier_id_solar' => $supplier_id_solar,
			'supplier_id_tangki' => $supplier_id_tangki,
			'supplier_id_sc' => $supplier_id_sc,
			'supplier_id_genset' => $supplier_id_genset,
			'supplier_id_wl' => $supplier_id_wl,
			'supplier_id_timbangan' => $supplier_id_timbangan,
