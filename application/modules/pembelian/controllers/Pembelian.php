...
class Pembelian extends Secure_Controller
{
    ...
    public function table_penawaran_pembelian()
    {
        ...
        $this->db->where('tanggal_penawaran  >=', date('2023-08-01'));
    ...
    public function table_penagihan_pembelian()
    {
        ...
        if (!empty($w_date)) {
            ...
            $start_date = $arr_date[0];
            $end_date = $arr_date[1];
            $this->db->where('tanggal_invoice  >=', date('Y-m-d h:i A', strtotime($start_date.' 23:59:59')));
            $this->db->where('tanggal_invoice <=', date('Y-m-d h:i A', strtotime($end_date.' 23:59:59')));
        }
    ...
    public function hapus_penawaran_pembelian($id)
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            # Something went wrong.
            $this->db->trans_rollback();
            $this->session->set_flashdata('notif_error','<b>Gagal Hapus Penawaran Pembelian</b>');
            redirect('admin/pembelian');
        } else {
            # Everything is Perfect. 
            # Committing data to the database.
...
<?php
    ...
    public function table_pembayaran_penagihan_pembelian($id)
    {
        ...
        $query = $this->db->get_where('pmm_pembayaran_penagihan_pembelian', array('penagihan_pembelian_id' => $id));
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $key => $row) {
                $row['no'] = $key + 1;
                // $row['coa'] = '' 
    ...
    public function approve_payment()
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('pmm_pembayaran_penagihan_pembelian', ['status' => 'DISETUJUI'
        ]);
    ...
    public function closed_po($id)
    {
        ...
        $this->session->set_flashdata('notif_success', '<b>Pesanan Pembelian Closed</b>');
        redirect("admin/pembelian");
    }
